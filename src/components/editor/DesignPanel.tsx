  ...
  return (
    <div className="h-full overflow-y-auto p-4 space-y-6 bg-card">
      ...
      <Section title="QR Code">
        <FieldRow label="Show on resume">
          <button
            type="button"
            role="switch"
            aria-checked={showQrCode}
            onClick={() => onShowQrCodeChange?.(!showQrCode)}
            className={[
              "relative inline-flex h-5 w-9 items-center rounded-full transition-colors",
              showQrCode ? "bg-primary" : "bg-muted-foreground/30",
            ].join(" ")}
          >
            <span
              className={[
                "inline-block h-3.5 w-3.5 rounded-full bg-white shadow-sm transition-transform",
                showQrCode ? "translate-x-[18px]" : "translate-x-[3px]",
              ].join(" ")}
            />
          </button>
        </FieldRow>
        ...
        {showQrCode && (
          <div>
            <Label htmlFor="dp-qr-url">Destination URL</Label>
            ...
            <input
              ...
              placeholder="https://..."
              value={qrCodeUrl ?? ""}
              onChange={(e) => onQrCodeUrlChange?.(e.target.value || null)}
              disabled={!isPaid}
              className={[
                "h-8 w-full px-2 text-sm rounded border border-input bg-card",
