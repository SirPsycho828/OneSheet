const SVG_ATTRS =
  'xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:middle;margin-right:4px;margin-top:-2px"';
...
export function detectContactType(segment: string): ContactIconType {
  ...
  if (/^[\d+(]/.test(segment) || /\d{3}.*\d{4}/.test(segment)) return 'phone';
  if (s.includes('http://') || /\.(com|io|dev|org|net)/.test(s)) return 'globe';
